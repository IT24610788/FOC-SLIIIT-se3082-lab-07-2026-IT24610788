...
int main(int argc, char **argv) {
    ...
    // SCAN: Computes cumulative/prefix reduction across ranks
    ...
    // Verification formula: K * (K + 1) / 2
    long long K = (long long)(rank + 1) * chunk_size;
    long long expected_prefix = K * (K + 1) / 2;
    printf("  Rank %d prefix verification: %s\n", rank, prefix_sum == expected_prefix ? "PASS" : "FAIL");

    if (rank == size - 1) {
        double elapsed = MPI_Wtime() - start;
        long long expected_total = (long long)N * (N + 1) / 2;

        printf("\n[Scan] Final sum (Rank %d) = %lld\n", rank, prefix_sum);
