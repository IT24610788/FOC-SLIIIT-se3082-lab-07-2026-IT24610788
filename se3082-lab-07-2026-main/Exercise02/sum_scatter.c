...
int main(int argc, char **argv) {
    ...
    /* Only root allocates the FULL array to save memory */
    int *array = NULL;
    if (rank == 0) {
    ...
    /* Every process allocates a local chunk buffer */
    int *local_chunk = (int *)malloc(chunk_size * sizeof(int));

    double start = MPI_Wtime();

    /* SCATTER: Root divides array and sends only the chunk to each process */
    ...
    printf("  Rank %d: summed local indices [0, %d) => local_sum = %lld\n", rank, chunk_size, local_sum);

    /* Collect results using point-to-point communication (unchanged for now) */
    if (rank != 0) {
